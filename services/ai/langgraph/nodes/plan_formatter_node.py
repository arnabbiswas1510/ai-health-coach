...
- **Weight Loss Accountability Card**: Include a prominent card inside Section 3 (inside `#retro-analysis`) labeled **"Weight Loss Accountability Partner"**. This card must dynamically display the start weight, target weight, current projected weight, actual weight, and deviation (BEHIND or ON TRACK) based on the `{metrics_analysis}` and `{physiology_analysis}` weight contexts. Display a direct coaching message (encouraging if on track, direct/firm if behind schedule due to inconsistencies or dietary issues) to serve as a strict accountability partner.
